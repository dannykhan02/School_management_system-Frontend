...
import { Edit, MapPin, Phone, Mail, School, BookOpen, CheckSquare, Square, GraduationCap, Award, AlertCircle, Users, Shield, ChevronDown, ChevronUp, Loader } from 'lucide-react';
...
function SchoolProfile() {
  ...
  return (
    <div className="w-full space-y-4 sm:space-y-6 p-3 sm:p-4 md:p-6 lg:p-8">
      ...
      {/* Curriculum Levels Section - Updated to neutral gray */}
      <div className="bg-white dark:bg-slate-800/50 rounded-lg sm:rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
        <div className="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-700">
          ...
          {curriculumLevels.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {curriculumLevels.map((level, index) => (
                <div key={index} className="p-4 border border-slate-200 dark:border-slate-600 rounded-lg transition-colors hover:bg-slate-50 dark:hover:bg-slate-700/20">
                    ...
                    <div className="mt-3 space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {schoolData.senior_secondary_pathways.map((pathway, idx) => (
                          <span key={idx} className="px-2 py-1 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs rounded-full flex items-center gap-1">
      ...
      {/* Grade/Class Levels Section - Updated to neutral gray */}
                  ...
                  {Object.keys(gradeLevelsByCurriculum[curriculum]).length > 0 ? (
                    <div className="space-y-4">
                      {Object.keys(gradeLevelsByCurriculum[curriculum]).map(level => (
                        <div key={level} className="p-4 border border-slate-200 dark:border-slate-600 rounded-lg transition-colors hover:bg-slate-50 dark:hover:bg-slate-700/20">
